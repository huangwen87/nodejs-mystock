...
function peopleCtrl($scope, $http, $templateCache){
  ...
  $scope.unwatchPeo=function(){
	...
	//股票实时数据
	function ajaxStock(){
		$scope.method = 'JSONP';  //跨域，如使用get或post都不能成功
    ...
    if(stockCode!==""){
     ...
     // $scope.url = 'http://xueqiu.com/stock/quote.json?code=SH601519&callback=JSON_CALLBACK'; //这个测试成功
  ...
  */
  ...
  var angular=stock.angular($http,$scope,myName,selfName);  //这个函数在js/tgy/topic.js中
  //自定义加载内容
